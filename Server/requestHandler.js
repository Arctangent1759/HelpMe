...
handle={
  "/":function(req,res,auth,helpMeDb){
	var karma_callback = function(data){
	  ...
	  res.write('<!doctype html><html><head><style>body{background-image:url("http://subtlepatterns.com/patterns/wild_oliva.png");}h1{font-family: sans-serif;text-align:center;}div.header{font-family: sans-serif;text-align:center;border-radius:35px;width:50%;padding:30px;margin:15px;background:rgba(200,200,200,.7);}div.content{font-family: sans-serif;text-align:center;border-radius:35px;width:50%;padding:30px;margin:15px;background:rgba(200,200,200,.7);}table{font-family: sans-serif;text-align:center;width:100%;} </style></head><body><center><div class="header"><h1>Leaderboard</h1></div><div class="content"><table><tr><th>Username</th><th>Karma</th></tr>');
	  for (var i = 0; i < data.length; i++){
		res.write("<tr><td>"+data[i].username+"</td><td>"+Math.floor(data[i].karma)+"</td></tr>")
	  }
	  res.write('</table></div></center></body></html>');
	  res.end();
	}
